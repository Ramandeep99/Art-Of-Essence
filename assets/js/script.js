...
class ClassAction {
    constructor(){
     ...
     timer(i, type){
        ...
        window.timerId = setInterval(timerStart, 30); // 30
         function timerStart(){
            this.reBidStart = false;
...
socket.onmessage = function (event) {
    ...
    switch(parsed.type){
        ...
        case 'inWait':
            ...
            $(".circle-content").html(`<h6>Bid starts in:</h6><h5 id='endtime'></h5>`);
        ...
        case 'bidStart':
            ...
            activeBorder.css('background-color', '#39B4CC');
            activeBorder.css('background-image','linear-gradient(90deg, transparent 50%, #A2ECFB 50%),linear-gradient(90deg, #A2ECFB 50%, transparent 50%)');
            $(".circle-content").html(`<p>Current Price: </p> <p style="font-weight: bold;">${parsed.content.current_price}$</p>`);
            // obj.clockAudio('play');
            break;
        case 'bonusTime':
            obj.mode = 'bonusTime';
            ...
            $(".circle-content").html('<h5>Bonus time!</h5>');
        ...
        case 'itemSold':
            ...
            $(".circle-content").html('<h5>Item sold!</h5>');
        ...
        case 'nextItem':
            ...
            obj.mode = 'nextItem';
            document.querySelector('.circle-bid').style.backgroundColor = '#E6F4F7';
            $(".circle-content").html('<h5>Next item</h5>');
            // obj.clockAudio('pause');
            // obj.nextItemAudio('play');
            console.log(parsed.content.winner)
            if(parsed.content.winner.length>0){
                $(".winner").html(`<p style="font-weight: bold;">Winner for the current Painting is:  ${parsed.content.winner[0].name}</p>`)
            }
            else{
                $(".winner").html(`<p style="font-weight: bold;">Painting Remains Unsold! </p>`)
            }
        ...
        case 'bidPlaced':
            ...
            // console.log(parsed.content.previousBids)
            console.log(parsed.content.previousBidsUser)
            previousBids.html(`<div><p>${parsed.content.previousBidsUser[parsed.content.previousBidsUser-1].name}</p><h6>${parsed.content.current_price}$</h6></div>${content}`);
            obj.clockAudio('pause');
